 ...
 */
@RunWith(AndroidJUnit4.class) @LargeTest public class AddNoteScreenTest {
   ...
   */
  @Rule public IntentsTestRule<AddNoteActivity> mAddNoteIntentsTestRule =
  ...
  @Before public void registerIdlingResource() {
    ...
    //Espresso.registerIdlingResources( //FIXME: registerIdlingResources / unregisterIdlingResources is deprecated
    //    mAddNoteIntentsTestRule.getActivity().getCountingIdlingResource());
  }

  @Test public void addImageToNote_ShowsThumbnailInUi() {
    ...
    onView(withId(R.id.add_note_image_thumbnail)).perform(scrollTo()) // Scroll to thumbnail
        .check(
            matches(allOf(hasDrawable(), // Check ImageView has a drawable set with a custom matcher
                isDisplayed())));
  }

  //FIXME android.support.test.espresso.NoMatchingViewException: No views in hierarchy found matching: with text: is "Notes cannot be empty"
  //@Test public void errorShownOnEmptyMessage() {
  ...
  //  //onView(allOf(withId(R.id.snackbar_text), withText(emptyNoteMessageText))).check(
  //  //    matches(isDisplayed()));
  //  //onView(withText(emptyNoteMessageText)).check(matches(withEffectiveVisibility(ViewMatchers.Visibility.VISIBLE)));
  //}
