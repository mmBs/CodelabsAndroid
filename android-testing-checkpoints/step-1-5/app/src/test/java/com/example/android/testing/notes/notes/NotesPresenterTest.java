...
public class NotesPresenterTest {
  ...
  private static List<Note> NOTES =
      Lists.newArrayList(new Note("Title1", "Description1"), new Note("Title2", "Description2"));

  private static List<Note> EMPTY_NOTES = new ArrayList<>(0);

  @Mock private NotesRepository mNotesRepository;

  @Mock private NotesContract.View mNotesView;

  /**
   * {@link ArgumentCaptor} is a powerful Mockito API to capture argument values and use them to
   * perform further actions or assertions on them.
   *
   * We have defined an ArgumentCaptor that captures parameters - we are using it to capture the
   * callback for the NotesRepository when data is loaded by the presenter, so we can call it
   * ourselves to simulate that the notes have been loaded already.
   */
  @Captor private ArgumentCaptor<LoadNotesCallback> mLoadNotesCallbackCaptor;

  private NotesPresenter mNotesPresenter;

  @Before public void setupNotesPresenter() {
  ...
  @Test public void loadNotesFromRepositoryLoadIntoViewAndHideEmptyListTextView() {
    ...
    //
    // Then progress indicator is hidden notes are shown in UI and Empty List TextView is hidden.
    verify(mNotesView).setProgressIndicator(false);
    verify(mNotesView).showNotes(NOTES);
    verify(mNotesView).setEmptyListText(false);
  }

  @Test public void loadEmptyNotesFromRepositoryAndShowEmptyListTextView() {
    mNotesPresenter.loadNotes(true);
    verify(mNotesRepository).getNotes(mLoadNotesCallbackCaptor.capture());

    mLoadNotesCallbackCaptor.getValue().onNotesLoaded(EMPTY_NOTES);
    verify(mNotesView).setProgressIndicator(false);
    verify(mNotesView).setEmptyListText(true);
  }

  @Test public void clickOnFab_ShowsAddsNoteUi() {
    //fail("Implement in step 6");
  ...
  @Test public void clickOnNote_ShowsDetailUi() {
